...
# ================================================================
# PDF REPORT STYLING
# Palette matches the frontend (indigo + purple + pink), so the
# downloaded report visually matches the CareerAI web app.
# ================================================================

BRAND_INDIGO   = (99, 102, 241)
BRAND_INDIGO_D = (67, 56, 202)
BRAND_PINK     = (236, 72, 153)
BRAND_PURPLE   = (139, 92, 246)
INK            = (30, 41, 59)
INK_MUTED      = (100, 116, 139)
CARD_BG        = (248, 250, 255)
CARD_BORDER    = (224, 227, 245)
TABLE_STRIPE   = (238, 242, 255)


class CareerPDF(FPDF):
    """FPDF subclass that draws a branded header/footer on every page."""
    ...
    def header(self):
        ...
        self.cell(0, 9, "CareerAI", ln=True)
        self.set_xy(12, 16)
        self.set_font("Arial", '', 10)
        self.set_text_color(224, 231, 255)
        self.cell(0, 6, "AI Career Recommendation Report")
    ...
    def footer(self):
        ...
        self.cell(0, 8, "Generated by CareerAI - this recommendation is guidance, not a guarantee.")
        self.set_xy(-35, -13)
        self.cell(25, 8, f"Page {self.page_no()}", align='R')


def draw_section_title(pdf, text, color=BRAND_INDIGO_D):
    """A small underlined section heading, CareerAI-style."""
    ...
    pdf.line(12, y, 12 + pdf.get_string_width(text) + 4, y)
    pdf.ln(4)
    pdf.set_text_color(*INK)


def draw_card(pdf, x, y, w, h, fill=CARD_BG, border=CARD_BORDER):
...
def draw_progress_bar(pdf, x, y, w, h, pct, color=BRAND_INDIGO):
    pdf.set_fill_color(226, 232, 240)
    pdf.rect(x, y, w, h, 'F')
    fill_w = max(0.0, min(pct, 100)) / 100 * w
    if fill_w > 0:
        pdf.set_fill_color(*color)
        pdf.rect(x, y, fill_w, h, 'F')


def draw_chip_row(pdf, x, y, items, color=BRAND_PURPLE, max_width=186):
    """Wraps a list of short strings into rounded 'chip' badges."""
    pdf.set_font("Arial", '', 9)
    light = tuple(min(c + 130, 255) for c in color)
    cx, cy = x, y
    for item in items:
        ...
        pdf.set_text_color(*color)
        pdf.set_xy(cx, cy)
        pdf.cell(tw, 6.5, item, border=1, align='C', fill=True)
        cx += tw + 3
    pdf.set_text_color(*INK)
    return cy + 10  # y-position after the last row
...
# ── /generate-pdf ────────────────────────────────────────────
# Redesigned to visually match the CareerAI frontend: branded header/
# footer on every page, a candidate profile card, a highlighted
# recommendation box with a confidence bar, skill chips, a zebra-striped
# metrics table, and a cleaner accuracy chart on its own page.
...
def generate_pdf(prediction_id):
    try:
        ...
        pdf.cell(95, 6, f"Report ID: #{pred.id}")
        pdf.cell(91, 6, f"Generated: {datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC", align='R', ln=True)
        pdf.ln(4)

        # ── Candidate profile card ───────────────────────
        draw_section_title(pdf, "Candidate Profile")
        card_y = pdf.get_y()
        draw_card(pdf, 12, card_y, 186, 30)
        fields = [
            ("Skills",     s(pred.skills)),
            ("Interest",   s(pred.interests)),
            ("CGPA",       s(pred.cgpa)),
            ("Experience", s(pred.experience)),
        ]
        col_w = 87
        for i, (label, value) in enumerate(fields):
            col_x = 18 if i % 2 == 0 else 18 + col_w
            row_y = card_y + 5 + (i // 2) * 11
            pdf.set_xy(col_x, row_y)
            pdf.set_font("Arial", 'B', 9)
            pdf.set_text_color(*INK_MUTED)
            pdf.cell(28, 6, label.upper())
            pdf.set_font("Arial", '', 10)
            pdf.set_text_color(*INK)
            val = value if len(value) < 45 else value[:42] + "..."
            pdf.cell(col_w - 28, 6, val)
        pdf.set_y(card_y + 36)

        # ── Recommended career highlight box ─────────────
        draw_section_title(pdf, "Top Recommended Career")
        box_y = pdf.get_y()
        draw_card(pdf, 12, box_y, 186, 34, fill=(238, 242, 255), border=BRAND_INDIGO)

        pdf.set_xy(18, box_y + 6)
        pdf.set_font("Arial", 'B', 16)
        pdf.set_text_color(*BRAND_INDIGO_D)
        pdf.cell(120, 9, s(pred.career_predicted))

        conf_pct = round((pred.confidence or 0) * 100, 1)
        pdf.set_font("Arial", 'B', 13)
        pdf.set_text_color(*BRAND_PINK)
        pdf.set_xy(150, box_y + 5)
        pdf.cell(36, 9, f"{conf_pct}%", align='R')
        pdf.set_font("Arial", '', 8)
        pdf.set_text_color(*INK_MUTED)
        pdf.set_xy(150, box_y + 13)
        pdf.cell(36, 5, "confidence", align='R')

        draw_progress_bar(pdf, 18, box_y + 23, 168, 4, conf_pct, color=BRAND_INDIGO)
        pdf.set_y(box_y + 40)

        pdf.set_font("Arial", size=10)
        pdf.set_text_color(*INK)
        pdf.multi_cell(0, 6, s(get_reason(pred.career_predicted)))
        pdf.ln(2)

        # ── Suggested skills as chips ────────────────────
        skills_list = get_suggested(pred.career_predicted)
        if skills_list:
            draw_section_title(pdf, "Suggested Skills to Learn")
            draw_chip_row(pdf, 12, pdf.get_y(), skills_list, color=BRAND_PURPLE)

        # ── Classifier performance (own page) ────────────
        if metrics:
            pdf.add_page()
            draw_section_title(pdf, "Classifier Performance (Test Set)")

            headers = ["Classifier", "Accuracy", "Precision", "Recall", "F1 Score"]
            col_w   = [56, 32.5, 32.5, 32.5, 32.5]
            pdf.set_fill_color(*BRAND_INDIGO)
            pdf.set_text_color(255, 255, 255)
            pdf.set_font("Arial", 'B', 10)
            for h, w in zip(headers, col_w):
                pdf.cell(w, 9, h, border=0, fill=True, align='C')
            pdf.ln()

            pdf.set_font("Arial", size=10)
            for i, (model_name, m) in enumerate(metrics.items()):
                fill = TABLE_STRIPE if i % 2 == 0 else (255, 255, 255)
                pdf.set_fill_color(*fill)
                pdf.set_text_color(*INK)
                for val, w in zip([
                    model_name,
                    ...
                    pdf.cell(w, 9, val, border='B', align='C', fill=True)
                pdf.ln()
            pdf.ln(6)

            # ── Accuracy chart ────────────────────────────
            draw_section_title(pdf, "Accuracy Comparison")
